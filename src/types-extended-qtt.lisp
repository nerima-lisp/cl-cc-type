...
(defun %multiplicity-symbol-name-p (value names)
  "Return T when VALUE is a symbol whose name package-independently matches one of NAMES."
  (and (symbolp value)
...
Symbol designators (ZERO, :one, OMEGA, :unrestricted, ...) match by name rather
than by EQUAL, so a bare symbol interned in any package normalizes the same
way a keyword does -- unlike EQUAL, which is package-sensitive for symbols."
  (cond
    ((eql value 0) :zero)
    ((eql value 1) :one)
    ((%multiplicity-symbol-name-p value '("ZERO")) :zero)
    ((%multiplicity-symbol-name-p value '("ONE")) :one)
    ((%multiplicity-symbol-name-p value '("OMEGA" "Ω" "UNRESTRICTED")) :omega)
...
(defun %semiring-has-identities-p (elements zero one)
  "T when ZERO and ONE are both members of ELEMENTS."
  (and (member zero elements :test #'equal)
       (member one elements :test #'equal)))

(defun %semiring-per-element-laws-p (elements add multiply zero one)
  "T when every element of ELEMENTS obeys additive/multiplicative identity
\(a+0=a, a*1=a\), stays closed under ADD/MULTIPLY with ZERO/ONE, and is
annihilated by ZERO under MULTIPLY."
  (every (lambda (a)
...
(defun %semiring-commutative-p (elements add multiply)
  "T when ADD is commutative and both ADD/MULTIPLY stay closed over
ELEMENTS, for every pair."
  (every (lambda (a)
           (every (lambda (b)
...
(defun %semiring-associative-distributive-p (elements add multiply)
  "T when ADD and MULTIPLY are each associative and MULTIPLY distributes
over ADD on both sides, for every triple in ELEMENTS."
  (every (lambda (a)
           (every (lambda (b)
                    (every (lambda (c)
...
(defun %semiring-preorder-valid-p (elements preorder)
  "T when PREORDER is absent, or is reflexive and transitive over ELEMENTS."
  (or (null preorder)
...
(defun finite-semiring-valid-p (semiring)
  ...
  (let ((elements (finite-semiring-elements semiring))
    ...
    (and (%semiring-has-identities-p elements zero one)
         (%semiring-per-element-laws-p elements add multiply zero one)
         (%semiring-commutative-p elements add multiply)
         (%semiring-associative-distributive-p elements add multiply)
         (%semiring-preorder-valid-p elements preorder))))
